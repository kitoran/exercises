    ...
    while(true) {
        ...
        if(!consume && mode == itersMode) {
            if(event.type == ButtonPress || event.type == MotionNotify) {
                double x, y;
                double xn, yn;
                picToNumDD(event.xbutton.x, event.xbutton.y, &x, &y);
                for(int i = 0; i < 10; i++) {
                    xn = x; yn = y;
                    iter(&x, &y);
...
//                sel = true;
                ...
                picToNumDD(startx, starty,
                         &sx, &sy);
                double ex, ey;
                picToNumDD(event.xbutton.x, event.xbutton.y,
                         &ex, &ey);
...
//                sel = false;
            ...
            if(event.type == ButtonRelease) {
                double mouseX = event.xbutton.x;
                double mouseY = event.xbutton.y;
                /*
